#include "mode.h"
#define tra(x,v,y) for(int x = st[y],v; x != 0; x = lk[x]) if((v = ve[x]) != fa[y])
using namespace std;
const int MN = 100010;
int k,n,a[MN],tot,st[MN],lk[MN*2],ve[MN*2],fa[MN],q[MN],ans[4],Ans[4];
int sum[MN],num[MN],a0[MN],a1[MN],size[MN];
...
void up(int x)
{
     int t1 = 0,t2 = 0;num[x] = 0;sum[x] = 0;
     int y = ((a[x]&(1<<(k-1))))?1:0;size[x] = 1;
     ...
     if(y)
     {
      ...
      ans[3]+=2*t1+1;
      tra(i,v,x) ans[3] += 2*(t1-a0[v])*a0[v],ans[3]+=2*(t2-a1[v])*a1[v],t1-=a0[v],t2-=a0[v];
     }
     else
     {
      t1 = 0,t2 = 0;
      ...
      ans[3]+=2*t2;
      tra(i,v,x) ans[3] += (t1-a0[v])*a1[v],ans[3]+=(t2-a1[v])*a0[v],t1-=a0[v],t2-=a0[v];
     }
}
