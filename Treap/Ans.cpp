...
void roll(int x)
{
     ...
     for(int i = 1;i<=3;i++)
      memcpy(ch[i],t[f[i]].tg,sizeof(no));
     ch[1][0] = ch[2][0];
     ch[2][0] = f[1];
     for(int i = 1;i<=2;i++)
      if(ch[3][i] == f[2])
     ...
     for(int i = 1;i<=3;i++)
      memcpy(t[f[i]].tg,ch[i],sizeof(t[f[i]].tg));
}
int ace[maxn],len=0;
int insert(int x,int y)
{
 ...
 for(int i = len;i>=1;i--)
 {
  if(t[ace[i]].x < y && t[t[ace[i]].tg[2]].y > t[ace[i]].y)
   roll(t[ace[i]].tg[2]),swap(ace[i],ace[i+1]);
  else if(t[ace[i]].x >= y && t[t[ace[i]].tg[1]].y > t[ace[i]].y)
   roll(t[ace[i]].tg[1]),swap(ace[i],ace[i+1]);
 }
...
int find(int x,int y)
{
     if(x == 0)
      return 0;
     if(t[x].x < y)
      return find(t[x].tg[2],y);
     if(t[x].x > y)
      return find(t[x].tg[1],y);
     if(t[x].x == y)
      return x;
}
void show(int x)
