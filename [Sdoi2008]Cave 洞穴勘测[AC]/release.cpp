...
#define link(x,y) access(x); x->mkrt();\
x->ch[2] = y;\
\

#define cut(x,y)access(x);x->pd();splay(y);y->pd();\
if(x->ch[2] != y || !y->fd(x))\
 {access(y);y->pd(); x->ch[2] = (node * )NULL;y->ch[y->fdc(x)] = (node *)NULL;return;}\
splay(x);x->pd();\
y->ch[2] = (node *) NULL;\
...
#define query(x,y) x = access(x);y = access(y);\
return (x == y);\
\
