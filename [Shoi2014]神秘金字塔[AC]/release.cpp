...
inline int & subto(int & a,const int & b){a = sub(a,a,b);return a;}
struct status
{
  LL h;
...
template<class _key,class _val,int S1,int S2>
struct hash
{
 public:
  ...
  _val *operator[](const status & sta)
  {
   int id = sta%HM;
...
typedef hash<status,int,1000003,100000> ha;
